...
from mahjong.hand_calculating.yaku_list import AkaDora, Riichi, Ippatsu, Chankan, Rinshan, Haitei, Houtei, \
    ...
    SanKantsu, SanshokuDoukou, Chiitoitsu, Shosangen, Honitsu, Junchan, Ryanpeikou, Chinitsu, Tsumo, Dora, Hana, \
    Shousharin, SanRenkou, Sanpuukou
from mahjong.hand_calculating.yaku_list.yakuman import KokushiMusou, ChuurenPoutou, Suuankou, Daisangen, Shousuushii, \
    ...
    Tenhou, Chiihou, RenhouYakuman, Daisharin, SuuRenkou, Manhon, NagashiYakuman, NagashiTanyao
...
class YakuConfig(object):
    ...
    def __init__(self):
        ...
        self.hana = Hana(next(id))

        # Local
        self.shousharin = Shousharin(next(id))
        self.sanrenkou = SanRenkou(next(id))
        self.suurenkou = SuuRenkou(next(id))
        self.sanpuukou = Sanpuukou(next(id))
        self.manhon = Manhon(next(id))
        self.nagashi_yakuman = NagashiYakuman(next(id))
        self.nagashi_tanyao = NagashiTanyao(next(id))