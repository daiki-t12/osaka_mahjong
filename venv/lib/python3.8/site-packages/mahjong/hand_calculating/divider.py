...
class HandDivider(object):
    ...
    def divide_hand(self, tiles_34, melds=None):
        ...
        # 4枚使い七対子
        kan_indices = self.find_kans(closed_hand_tiles_34)
        if kan_indices:
            just_pair_indices = self.find_just_pair(closed_hand_tiles_34)
            hand = []
            for index in just_pair_indices:
                hand.append([index] * 2)
            for index in kan_indices:
                hand.append([index] * 2)
                hand.append([index] * 2)
            hands.append(hand)
    ...
    def find_kans(self, tiles_34, first_index=0, second_index=33):
        """
        Find all possible kans in the hand and return their indices
        :return: array of kan indices
        """
        kan_indices = []
        for x in range(first_index, second_index + 1):
            if tiles_34[x] == 4:
                kan_indices.append(x)

        return kan_indices

    def find_just_pair(self, tiles_34, first_index=0, second_index=33):
        """
        Find all possible pair in the hand and return their indices
        :return: array of pair indices
        """
        just_pair_indices = []
        for x in range(first_index, second_index + 1):
            if tiles_34[x] == 2:
                just_pair_indices.append(x)

        return just_pair_indices
