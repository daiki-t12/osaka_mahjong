...
class HandCalculator(object):
    ...
    def estimate_hand_value(self, tiles=None, win_tile=None, hana=0, melds=None, dora_indicators=None, config=None):
        """
        ...
        # special situation
        if self.config.is_nagashi_yakuman:
            hand_yaku.append(self.config.yaku.nagashi_yakuman)
            fu = 30
            han = self.config.yaku.nagashi_yakuman.han_closed
            cost = scores_calculator.calculate_scores(han, fu, self.config, True)
            return HandResponse(cost, han, fu, hand_yaku)

        if self.config.is_nagashi_tanyao:
            hand_yaku.append(self.config.yaku.nagashi_tanyao)
            fu = 30
            han = self.config.yaku.nagashi_tanyao.han_closed
            cost = scores_calculator.calculate_scores(han, fu, self.config, True)
            return HandResponse(cost, han, fu, hand_yaku)
        ...
        for hand in hand_options:
            ...
            valued_tiles = [HAKU, HATSU, CHUN, NORTH, self.config.player_wind, self.config.round_wind]
            ...
            for win_group in win_groups:
                ...
                if is_chiitoitsu:
                    # 大車輪
                    is_daisharin = self.config.yaku.daisharin.is_condition_met(hand,self.config.options.has_daisharin_other_suits)
                    # 小車輪
                    is_shousharin = self.config.yaku.shousharin.is_condition_met(hand)
                    if is_daisharin:
                        if self.config.options.has_daisharin:
                            self.config.yaku.daisharin.rename(hand)
                        hand_yaku.append(self.config.yaku.daisharin)
                    elif is_shousharin:
                        hand_yaku.append(self.config.yaku.shousharin)
                    else:
                ...
                # 小車輪と混一色は複合しない
                if self.config.yaku.honitsu.is_condition_met(hand) and not is_shousharin:
                    hand_yaku.append(self.config.yaku.honitsu)

                # 大車輪と清一色は複合しない
                if self.config.yaku.chinitsu.is_condition_met(hand) and not is_daisharin:
                    hand_yaku.append(self.config.yaku.chinitsu)

                if self.config.yaku.manhon.is_condition_met(hand):
                    hand_yaku.append(self.config.yaku.manhon)
                ...
                if len(pon_sets):
                    ...
                    # 連刻
                    if self.config.yaku.sanrenkou.is_condition_met(hand):
                        hand_yaku.append(self.config.yaku.sanrenkou)

                    if self.config.yaku.suurenkou.is_condition_met(hand):
                        hand_yaku.append(self.config.yaku.suurenkou)
                    ...
                    # 北は常時役牌
                    if self.config.yaku.north.is_condition_met(hand):
                        hand_yaku.append(self.config.yaku.north)

                    if self.config.yaku.daisangen.is_condition_met(hand):
                        hand_yaku.append(self.config.yaku.daisangen)

                    if self.config.yaku.sanpuukou.is_condition_met(hand):
                        hand_yaku.append(self.config.yaku.sanpuukou)
                ...
                if not yakuman_list:
                    ...
                    if count_of_hana:
                        self.config.yaku.hana.han_open = count_of_hana
                        self.config.yaku.hana.han_closed = count_of_hana
                        hand_yaku.append(self.config.yaku.hana)
                        han += count_of_hana
